                                ...
                                const libvlc_media_thumbnail_params_t* p_params )
{
    ...
    if ( unlikely( req == NULL ) )
        return NULL;
    req->p_md = p_md;
    req->i_width = p_params->i_width;
    req->i_height = p_params->i_height;
    req->i_type = p_params->i_picture_type;
    libvlc_media_retain( p_md );
    vlc_thumbnailer_params_t params;
    if ( p_params->i_seek_type == libvlc_media_thumbnail_seek_time )
    {
        params.i_type = VLC_THUMBNAILER_SEEK_TIME;
        params.i_time = VLC_TICK_FROM_MS( p_params->i_time );
    }
    else
    {
        assert( p_params->i_seek_type == libvlc_media_thumbnail_seek_pos );
        params.i_type = VLC_THUMBNAILER_SEEK_POS;
        params.f_pos = p_params->f_pos;
    }
    params.b_fast_seek = p_params->b_fast_seek;
    params.p_input_item = p_md->p_input_item;
    params.i_timeout = VLC_TICK_FROM_MS( p_params->i_timeout );
    params.pf_cb = media_on_thumbnail_ready;
    params.p_user_data = req;
    req->p_req = vlc_thumbnailer_Request( p_priv->p_thumbnailer, &params );
    if ( req->p_req == NULL )
    {
        free( req );
        libvlc_media_release( p_md );
