...
static void thumbnailer_callback( void* data, picture_t* p_thumbnail )
{
    ...
    if ( p_thumbnail != NULL )
    {
        ...
        assert( p_thumbnail->format.i_chroma == VLC_CODEC_RGB32 );
        ...
        assert( p_thumbnail->date == expected_date && "Unexpected picture date");
#endif
    }
    else
...
static void test_thumbnails( libvlc_instance_t* p_vlc )
{
    ...
    for ( size_t i = 0; i < sizeof(test_params) / sizeof(test_params[0]); ++i)
    {
        ...
        {
            params.f_pos = test_params[i].f_pos;
            params.i_type = VLC_THUMBNAILER_SEEK_POS;
        }
        else
        {
            params.i_time = test_params[i].i_time;
            params.i_type = VLC_THUMBNAILER_SEEK_TIME;
        }
        params.b_fast_seek = test_params[i].b_fast_seek;
        params.p_input_item = p_item;
        params.pf_cb = thumbnailer_callback;
        params.i_timeout = test_params[i].i_timeout;
        params.p_user_data = &ctx;

        vlc_mutex_lock( &ctx.lock );
        int res = 0;
        vlc_thumbnailer_Request( p_thumbnailer, &params );
...
static void test_cancel_thumbnail( libvlc_instance_t* p_vlc )
{
    ...
    vlc_thumbnailer_params_t params;
    params.i_time = VLC_TICK_FROM_SEC( 1 );
    params.i_type = VLC_THUMBNAILER_SEEK_TIME;
    params.b_fast_seek = false;
    params.p_input_item = p_item;
    params.pf_cb = thumbnailer_callback_cancel;
    params.i_timeout = VLC_TICK_INVALID;
    params.p_user_data = &ctx;

    vlc_mutex_lock( &ctx.lock );
    int res = 0;
    vlc_thumbnailer_request_t* p_req = vlc_thumbnailer_Request( p_thumbnailer,
                                                                &params );
